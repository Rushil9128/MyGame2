...
function setup() {
  ...
  //bob spawn = 120,600
  bob = createSprite(975,195, 50, 50);
...
function draw() {
  ...
  if(noteTaken===true){
    text("YOU CAN TELEPORT THROUGH A CERTAIN WALL BY STANDING BY IT FOR 10 SEC",0,55);
  }
  if(gameState==="Stage3"){
    text("KEY FOUND , NOW ESCAPE",0,55);
  }
  if(hasMoney===true){
    text("YOU HAVE MONEY , FIND THE LIFT KEY",0,25);
  }
  ...
  if(bob.isTouching(note)&&keyDown(UP_ARROW)){
    noteTaken=true;
    note.visible=false;
  }
...
function mouseClicked(){
    if((bob.x>915&&bob.x<1020)&&(bob.y<520&&bob.y>410)&&gameState==="Stage2"){
      gameState="Stage3";
      hasMoney=false;
    }
}